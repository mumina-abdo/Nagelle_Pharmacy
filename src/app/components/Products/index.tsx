...
interface ProductCardProps {
  icon: ReactNode; // icon will be a JSX element (ReactNode)
  title: string;
  description: string;
}
...
const ProductCard: React.FC<ProductCardProps> = ({ icon, title, description }) => (
  <div className="bg-white p-6 rounded-lg shadow-md flex flex-col items-center text-center transition-all duration-300 hover:bg-blue-500 hover:shadow-xl hover:text-white">
    ...
    <h3 className="text-xl font-semibold mb-2 transition-colors duration-300 hover:text-white">
      {title}
    </h3>
    <p className="text-gray-600 text-sm transition-colors duration-300 hover:text-white">
      {description}
    </p>
  </div>
);
